...
| `0095_hr_integration.sql` | The HR Payroll app connected (see `docs/hr-integration.md`): `employee_claims` booked Dr mapped head / Cr 2770 Employee Claims Payable, paid in one cash/bank voucher by `pay_employee_claims()` and reported back; `hr_branch_map`, `hr_claim_heads`, `hr_inbound_events` (webhook replay guard), `hr_links`; service-role-only `hr_receive_event()` / `hr_sync_batch()`; `import_hr_payroll()` into draft payroll runs (`payroll_runs.source = HR`); permissions `hr.claims.view` / `hr.claims.pay` |
