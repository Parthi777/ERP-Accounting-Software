...
export const metadata: Metadata = { title: 'Spares' };
export const dynamic = 'force-dynamic';

export default async function Page() {
  const context = await requireTenantContext();
  const rows = await getInventoryItems('SPARE');
  ...
  const columns: Column<InventoryItemRow>[] = [
    ...
    { key: 'status', header: 'Status', render: (r) => <Badge variant={r.status === 'ACTIVE' ? 'positive' : 'neutral'}>{r.status}</Badge> },
    {
      ...
      render: (r) => (
        <RowActions kind="inventory_item" id={r.id} label={r.name} editHref={`/masters/spares/${r.id}/edit`} canManage={canManage} />
      ),
    },
  ];
    ...
    <div>
      <PageHeader
        ...
        action={
          canManage ? (
            <Button asChild>
              <Link href="/masters/spares/new"><Plus aria-hidden />New item</Link>
            </Button>
          ) : undefined
        }
      />
