...
import {
  commitVehicleImportAction,
  previewVehicleImportAction,
} from '@/server/services/vehicles/actions';
import type { ImportPreview } from '@/server/services/vehicles/vehicle-service';

/**
 * Vehicle stock upload — spec §14.
 *
 ...
 * landed without checking the stock list row by row.
 */
export function StockUpload() {
  ...
  const read = (file: File) => {
    ...
    reader.onload = () => {
      ...
      startTransition(async () => {
        setPreview(await previewVehicleImportAction(text));
      });
    };
  ...
  const confirm = () => {
    ...
    setError(null);
    startTransition(async () => {
      const outcome = await commitVehicleImportAction(csv);
      if (!outcome.ok) {
    ...
    return (
      <Panel className="p-6 text-center">
        ...
        <p className="mt-1 text-sm text-ink-600">
          Each one is now in stock as a separate chassis, with its movement log started.
        </p>
        <div className="mt-5 flex justify-center gap-2">
          <Button onClick={() => router.push('/vehicles')}>View stock</Button>
      ...
      >
        <div className="text-center">
          ...
          <p className="mt-1 text-xs text-ink-500">
            Required columns: chassis_no, engine_no, model_code, branch_code, purchase_invoice, purchase_cost.
            Optional: variant_code, colour, key_no, purchase_date.
          </p>
          ...
          <div className="mt-4 flex justify-center gap-2">
            ...
            <Button variant="secondary" asChild>
              <a href="/vehicle-stock-template.csv" download>Download template</a>
            </Button>
          ...
          {preview.errorCount > 0 && (
            <div className="flex items-start gap-2 rounded-lg border border-warning-200 bg-warning-50 px-3 py-2 text-sm text-warning-800">
              ...
              <span>
                ...
                a partial import would leave you unsure which vehicles actually landed.
              </span>
            </div>
          )}
          ...
          <SolidPanel className="overflow-hidden">
            <div className="table-sticky overflow-auto" style={{ maxHeight: '32rem' }}>
              <table className="w-full border-collapse text-sm">
                ...
                <thead>
                  <tr>
                    {['Row', 'Chassis', 'Engine', 'Model', 'Variant', 'Branch', 'Invoice', 'Cost', 'Result'].map((h) => (
                ...
                <tbody>
                  {preview.rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 font-mono text-xs">{row.chassis_no || '—'}</td>
                      <td className="px-3 py-2 font-mono text-xs">{row.engine_no || '—'}</td>
                      <td className="px-3 py-2">{row.model_code || '—'}</td>
                      <td className="px-3 py-2">{row.variant_code || '—'}</td>
                      <td className="px-3 py-2">{row.branch_code || '—'}</td>
                      <td className="px-3 py-2">{row.purchase_invoice || '—'}</td>
                      <td className="numeric px-3 py-2">{row.purchase_cost || '—'}</td>
